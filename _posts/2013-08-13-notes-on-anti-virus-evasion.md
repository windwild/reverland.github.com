...
To find out the 'bad string', I use the old bisection method to locate where the signature really is:
    ...
    cat payload_raw | cut -c614-663 > macilious.txt

Ok, then I get the 'bad string' into `macilious.txt`. Scan it with Avast!, Avast! flag it as malware. You can download it [here](macilious.txt) and test it with your own avast.
